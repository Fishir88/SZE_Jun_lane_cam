...
class LaneDetector(Node):
    ...
    def sliding_window(self, edges):
        ...
        leftx_base = 1 #np.argmax(histogram[:midpoint])
        rightx_base = width #np.argmax(histogram[midpoint:]) + midpoint
        '''
        ...
        if len(leftx) > 0 and len(lefty) > 0:
            with warnings.catch_warnings():
                ...
                try:
                    left_fit = np.polyfit(lefty, leftx, 3)  # Change degree to 3
        ...
        if len(rightx) > 0 and len(righty) > 0:
            with warnings.catch_warnings():
                warnings.simplefilter('error', np.RankWarning)
                try:
                    right_fit = np.polyfit(righty, rightx, 3)  # Change degree to 3
                except np.RankWarning:
                    self.get_logger().warning("Right lane polyfit may be poorly conditioned")

        ploty = np.linspace(0, edges.shape[0] - 1, edges.shape[0])
        left_fitx = (left_fit[0] * ploty**3 + left_fit[1] * ploty**2 + left_fit[2] * ploty + left_fit[3]) if left_fit is not None else None
        right_fitx = (right_fit[0] * ploty**3 + right_fit[1] * ploty**2 + right_fit[2] * ploty + right_fit[3]) if right_fit is not None else None
